#!/usr/bin/env python3
"""
SiteBoss API - Debug version with increased timeouts
"""
import asyncio
import argparse
import json
...
async def pull_xml_data(host: str, username: str, password: str) -> str:
    """Pull XML data from SiteBoss device using Playwright with debug info"""
    base = f'http://{host}'
    ...
    async with async_playwright() as p:
        ...
        try:
            # 1) Open login page with longer timeout
            print(f"   Navigating to: {base}/UnitLogin.html")
            await page.goto(f'{base}/UnitLogin.html', wait_until='domcontentloaded', timeout=60000)
            ...
            await page.evaluate(
                ...
                const response = await fetch('/index.html?commit=login', {
                    method:'POST',
            ...
            print(f"   Navigating to: {base}/UnitMain.html")
            await page.goto(f'{base}/UnitMain.html', wait_until='domcontentloaded', timeout=60000)
            print("   ✅ Main page loaded")
            
            # 4) Navigate directly to XML and read response text
            print("📥 Fetching XML data...")
            print(f"   Requesting: {base}/SiteStatus.xml")
            
            resp = await page.goto(f'{base}/SiteStatus.xml', timeout=60000)
            if resp:
                print(f"   Response status: {resp.status}")
                xml_text = await resp.text()
                print(f"   Response length: {len(xml_text)} characters")
            else:
                # Fallback: try to get content directly from page
                print("   No response object, trying direct page content...")
                xml_text = await page.content()
                print(f"   Page content length: {len(xml_text)} characters")
            ...
            if xml_text.strip().startswith('<?xml'):
                ...
                print(f"❌ Got non-XML response. First 200 chars: {xml_text[:200]}")
...
async def main():
    parser = argparse.ArgumentParser(description='SiteBoss API Debug - Pull and convert data for backend')
    ...
    parser.add_argument('--output', default='siteboss_debug_data.json', help='Output JSON filename')
    parser.add_argument('--save-xml', action='store_true', help='Also save raw XML file')
    args = parser.parse_args()

    try:
        print("🚀 SiteBoss API Debug Data Puller")
        print(f"   Target: {args.host}")
        print(f"   Output: {args.output}")
        print(f"   Timeout: 60 seconds per operation")
